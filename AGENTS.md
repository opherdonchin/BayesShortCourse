...
- Treat predictor support and outcome support as separate plotting decisions. A discrete outcome does **not** make a continuous predictor discrete. Preserve a continuous numeric x-axis and continuous relationship for a continuous predictor/model, even when observations exist at only a few predictor values. Use pointwise or categorical geometry only when the predictor or modeled relationship itself is discrete; do not let the response support erase predictor continuity.
